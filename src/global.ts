exports.phrasesLeft = (getName, sab2, encodeURIComponent) => {
  response = [
    ...
    `${encodeURIComponent(getName(sab2.participants[0]))}`,
    `${encodeURIComponent(getName(sab2.participants[0]))} de sair do grupo: ${encodeURIComponent(mdata_2.subject)}`,
    `Nem gostava de você, eu só digo aleluia por tu ter saído :)`,
    `Nunca fui com a tua cara, digo glória por tu ter saído!`,
    `Nada pra ver aqui, ele(a) saiu por um 'acidente'...`,
  ];
...
exports.phrasesWelcome = (mdata_2, getName, sab2, encodeURIComponent) => {
  response = [
    ...
    `O membro ${encodeURIComponent(getName(sab2.participants[0].split('@')[0]))} acaba de cair de paraquedas aqui no grupo...`,
    `Leia a descrição do grupo para passar um tempo aqui...`,
    `Minhas saudações a você, novo membro desconhecido.`,
  ];
  return response[Math.floor(Math.random() * response.length)];
};
...
exports.igstalk = (data) => {
  // Variáveis:
  var isPrivateAccount =
    data.resultado.is_private === true
      ? 'Sim.'
      : data.resultado.is_private === false
        ? 'Não.'
        : undefined;
  var isMusicProfile =
    data.resultado.is_music_on_profile === true
      ? 'Sim.'
      : data.resultado.is_music_on_profile === false
        ? 'Não.'
        : undefined;
  var isAccountBusiness =
    data.resultado.is_business === true
      ? 'Sim.'
      : data.resultado.is_business === false
        ? 'Não.'
        : undefined;
  var isVerifiedAccount =
    data.resultado.is_verified === true
      ? 'Sim.'
      : data.resultado.is_verified === false
        ? 'Não.'
        : undefined;
  var isHighlight =
    data.resultado.is_highlight === true
      ? 'Sim.'
      : data.resultado.is_highlight === false
        ? 'Não.'
        : undefined;
  // Texto de envio:
